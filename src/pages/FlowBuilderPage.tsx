...
const NODE_PALETTE = [
  { type: 'trigger', label: 'Gatilho', icon: Play, color: 'text-green-600', bg: 'bg-green-500/10' },
  { type: 'message', label: 'Mensagem', icon: MessageSquare, color: 'text-blue-600', bg: 'bg-blue-500/10' },
  { type: 'menu', label: 'Menu', icon: List, color: 'text-indigo-600', bg: 'bg-indigo-500/10' },
  { type: 'condition', label: 'Condição', icon: GitBranch, color: 'text-amber-600', bg: 'bg-amber-500/10' },
  { type: 'delay', label: 'Atraso', icon: Clock, color: 'text-purple-600', bg: 'bg-purple-500/10' },
  { type: 'action', label: 'Ação', icon: Zap, color: 'text-red-600', bg: 'bg-red-500/10' },
  { type: 'question', label: 'Pergunta', icon: HelpCircle, color: 'text-teal-600', bg: 'bg-teal-500/10' },
  { type: 'randomizer', label: 'Randomizador', icon: Shuffle, color: 'text-cyan-600', bg: 'bg-cyan-500/10' },
  { type: 'aiassistant', label: 'Assistente IA', icon: Sparkles, color: 'text-violet-600', bg: 'bg-violet-500/10' },
  { type: 'subflow', label: 'Conectar fluxo', icon: GitMerge, color: 'text-fuchsia-600', bg: 'bg-fuchsia-500/10' },
];

// Cores dos nós no minimapa (mesma paleta dos cards)
const MINIMAP_COLORS: Record<string, string> = {
  trigger: '#22c55e', message: '#3b82f6', menu: '#6366f1', condition: '#f59e0b',
  delay: '#a855f7', action: '#ef4444', question: '#14b8a6', randomizer: '#06b6d4',
  aiassistant: '#8b5cf6', subflow: '#d946ef',
};
...
export default function FlowBuilderPage() {
  ...
  // Arrastar da paleta e soltar no canvas
  ...
  // Botão VOLTAR do navegador: o BrowserRouter troca de rota sem recarregar, então
  // o beforeunload não dispara e o fluxo não salvo era perdido. Ao entrar no editor,
  // empurramos uma entrada sentinela no histórico; o "voltar" consome a sentinela e
  // cai aqui: com alterações pendentes reabrimos a sentinela e mostramos o diálogo;
  // sem alterações, voltar leva à lista de fluxos.
  const dirtyRef = useRef(dirty);
  useEffect(() => { dirtyRef.current = dirty || saving; }, [dirty, saving]);
  useEffect(() => {
    if (listView) return;
    window.history.pushState({ flowEditorGuard: true }, '');
    const onPopState = () => {
      if (dirtyRef.current) {
        window.history.pushState({ flowEditorGuard: true }, '');
    ...
    <div ref={editorContainerRef} className={cn('flex flex-col bg-background', isFullscreen ? 'h-screen' : 'h-[calc(100vh-3.5rem)]')}>
      ...
      <div className="flex items-center gap-3 border-b border-border px-4 py-2 bg-card shrink-0">
        ...
        <Input
          ...
          className="h-8 text-sm font-semibold w-56 border-none bg-transparent focus-visible:ring-1 px-1.5"
        />
        {/* Status + auto-save pill */}
        <div
          className={cn(
            'flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium shrink-0',
            flowActive ? 'bg-green-500/10 text-green-700' : 'bg-amber-500/10 text-amber-700'
          )}
        >
          <span className={cn('h-1.5 w-1.5 rounded-full', flowActive ? 'bg-green-500' : 'bg-amber-500')} />
          {flowActive ? 'Ativo' : 'Rascunho'}
          {saving ? (
            ...
            <span className="flex items-center gap-1 text-muted-foreground">· <Check className="h-3 w-3 text-green-600" /> salvo</span>
          ) : null}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Side palette */}
        <div className="w-60 border-r border-border bg-card p-3 shrink-0 overflow-y-auto">
          <p className="text-xs font-semibold text-foreground px-1">Caixa de blocos</p>
          <p className="text-[10px] text-muted-foreground px-1 pb-2">Arraste para o canvas ou clique para adicionar</p>
            ...
            {NODE_PALETTE.filter(n => n.type !== 'trigger').map(item => {
              ...
              return (
                <button
                  ...
                  className="flex flex-col items-start gap-1.5 rounded-xl border border-border/70 bg-card p-2.5 text-left hover:border-primary/40 hover:shadow-sm transition-all cursor-grab active:cursor-grabbing"
                  title={`Adicionar ${item.label}`}
                >
                  <div className={`flex h-7 w-7 items-center justify-center rounded-lg ${item.bg}`}>
                    <Icon className={`h-3.5 w-3.5 ${item.color}`} strokeWidth={2} />
                  </div>
                  <span className="text-[11px] font-medium text-foreground/90 leading-tight">{item.label}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="flex-1 relative">
          ...
          >
            <Background gap={18} size={1.4} className="opacity-40" />
            <Controls
              orientation="horizontal"
              position="bottom-center"
              className="!flex-row !rounded-full !border !border-border !bg-card !shadow-md !overflow-hidden [&>button]:!border-0 [&>button]:!bg-transparent [&>button:hover]:!bg-accent"
            />
            <MiniMap
              pannable
              zoomable
