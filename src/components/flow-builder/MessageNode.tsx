  ...
  return (
    <div className="rounded-xl border-2 border-blue-500/40 bg-card px-4 py-3 shadow-sm min-w-[180px] max-w-[260px]">
      ...
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <p className="text-[10px] uppercase tracking-wider text-blue-600 font-semibold">Mensagem</p>
      </div>
      <p className="text-xs text-foreground font-medium truncate">{(data.label as string) || 'Mensagem'}</p>
      {content && <p className="text-[11px] text-muted-foreground mt-1 line-clamp-2">{content}</p>}
